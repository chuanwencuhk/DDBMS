...
#define TMP_FILE_PATH "/mnt/mem/"
//#define TMP_FILE_PATH "/home/yf-chai/ourddb/"
#include <deque>
#include <fstream>
#include <set>
#include <string.h>
...
string default_local_ip = "10.48.139.126";
...
void Dispatcher::set_treevector_from_querytree(struct query_tree& qtree)
{
	...
	//this->tree_vector.push_front(root_pos);
...
query_tree Dispatcher::get_querytree_fromSQL(string sql)
{
	...
	get_basic_opt_tree(original_tree, 2);//使用前3种优化
    ...
    while (!tree_vector.empty() )
    {
        ...
        case 0:
        {
            ...
            if(n != f_name.npos)
            {
                ...
                ofstream fs_out(TMP_FILE_PATH+get_hash_fromstr(f_name)+".ttb");
                fs_out<<res_str;
                fs_out.close();
                ...
                string buf_string = readFileIntoString(TMP_FILE_PATH + get_hash_fromstr(f_name)+".ttb");
                //string buf_string = readFileIntoString("/home/yf-chai/ourddb/"+get_hash_fromstr(f_name)+".ttb");
...
void Dispatcher::exec_SQL_query_poor_optimal(string SQL)
{
    ...
    MetadataManager* pmgr = MetadataManager::getInstance();
	cout<<"The SQL: "<<endl<<SQL<<endl<<" is sent to create querytree"<<endl;
    struct query_tree qtree = get_querytree_fromSQL(SQL);
    set_treevector_from_querytree(qtree);
    ...
    while (!tree_vector.empty() )
    {
        ...
        case 0:
        {
            ...
            {
				if(site_ID==2) continue;//just for cunstomer fragment 2
                //if(site_ID == -1) site_ID =1;
                string IP_address = pmgr->get_IP_by_siteID(site_ID-1);
                string sql_stm = "select * from "+f_name+";" ;
                cout<<"the SQL is: "<<sql_stm<<" to site: "<<site_ID<<" IP is: "<<IP_address<<endl;

                //exec RPC query and save store result in /mnt/mem 
				if(frg_set.count(f_name+to_string(site_ID-1)) == 0)
				{
                string res_str;
				frg_set.insert(f_name+to_string(site_ID-1));
                res_str = RPCExecuteQuery(IP_address, db_names2[site_ID-1],sql_stm);//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                ofstream fs_out(TMP_FILE_PATH+get_hash_fromstr(f_name)+".ttb");
                fs_out<<res_str;
                fs_out.close();
...
//to create complement table with vertical fragment
                if(hasdifferent_frag_attr || hasselectall)
                {
                    ...
                    string sql_stm = "select * from "+f_name+";" ;
                    cout<<"the complemnt_siteID SQL is: "<<sql_stm<<" to site: "<<complemnt_siteID<<" IP is: "<<IP_address<<endl;

                    //exec RPC query and save store result in ./tmp/
                    string res_str;
                    cout<<"db_names2[complemnt_siteID] "<<db_names2[complemnt_siteID]<<endl;
                    res_str = RPCExecuteQuery(IP_address,db_names2[complemnt_siteID], sql_stm);//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                    cout<<"res_str*************from complemnt_siteID "<<endl;
					ofstream fs_out(TMP_FILE_PATH+get_hash_fromstr(f_name)+"_L"+".ttb");
                    fs_out<<res_str;
                    fs_out.close();
                ...
                if(tmp_set.count(get_hash_fromstr(f_name)+"_L")==0)
                {
					...
					//string file_name = "/home/yf-chai/ourddb/";
					string file_name = TMP_FILE_PATH;
					file_name += get_hash_fromstr(f_name)+"_L"+".ttb";
					cout<<"file_name***************"<<endl;
                	string buf_string = readFileIntoString(file_name);
					cout<<"get_hash_fromstr(f_name)***********************"<<get_hash_fromstr(f_name)+"_L"<<endl;
                    //cout<<"buf_string********"<<buf_string<<endl;
                    RPCInsertFileToTable(default_local_ip,"db1",buf_string,get_hash_fromstr(f_name)+"_L");
        ...
        else if(frg.condition_slice[site_ID-1].con_H1.isValid ||frg.condition_slice[site_ID-1].con_H2.isValid )//no vertical fragment condition
            {
                ...
                //string sql_stm = "select * from "+f_name+";" ;
				string sql_stm = "select * from "+f_name;
				int father = qtree.node[pos].fa;
				if(qtree.node[father].type == 3)
				{ 
					sql_stm+= " where "+f_name+"."+qtree.node[father].str;
				}
					sql_stm+=";";
...
//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                ...
                string file_name = TMP_FILE_PATH+get_hash_fromstr(f_name)+".ttb";
                cout << file_name<<endl;
                string buf_string = readFileIntoString(file_name);
                //cout<<"buf_string********"<<buf_string<<endl;
                RPCInsertFileToTable(default_local_ip,"db1",buf_string,get_hash_fromstr(f_name));
    ...
    cout<<final_res<<endl;
	char* c_final_res = (char*)final_res.c_str();
	char* p = c_final_res;
	int rows = 0;
	while(*p != '\0'){
		if(*p=='\n')
