...
void MetadataManager::load_fragmentmetadata_fromcfg()
{
    ...
    for(int i=0;i<len;i++)
    {
        ...
        tmp.frag_table_name = frg_name;
        for(int j = 0; j<MAX_FRAGMENT_NUM;j++)
        {
            Setting& frgi_cfg = frg_info[CONFIG_NAME_FRAGMENT_SLICE+to_string(j)];
            frgi_cfg.lookupValue(CONFIG_NAME_FRAGMENT_ISVALID,tmp.condition_slice[j].isValid);

            {//for con_H1
                Setting& frg_conH1 = frgi_cfg[CONFIG_NAME_FRAGMENT_CON_H1];
                frg_conH1.lookupValue(CONFIG_NAME_FRAGMENT_ISVALID, tmp.condition_slice[j].con_H1.isValid);
                frg_conH1.lookupValue(CONFIG_NAME_FRAGMENT_ATTR_NAME, tmp.condition_slice[j].con_H1.attr_name);
                frg_conH1.lookupValue(CONFIG_NAME_FRAGMENT_ATTR_CONDITION, tmp.condition_slice[j].con_H1.attr_condition);
                frg_conH1.lookupValue(CONFIG_NAME_FRAGMENT_ATTR_VALUE, tmp.condition_slice[j].con_H1.attr_value);

            }

            {//for con_H2
                Setting& frg_conH2 = frgi_cfg[CONFIG_NAME_FRAGMENT_CON_H2];
                frg_conH2.lookupValue(CONFIG_NAME_FRAGMENT_ISVALID, tmp.condition_slice[j].con_H2.isValid);
                frg_conH2.lookupValue(CONFIG_NAME_FRAGMENT_ATTR_NAME, tmp.condition_slice[j].con_H2.attr_name);
                frg_conH2.lookupValue(CONFIG_NAME_FRAGMENT_ATTR_CONDITION, tmp.condition_slice[j].con_H2.attr_condition);
                frg_conH2.lookupValue(CONFIG_NAME_FRAGMENT_ATTR_VALUE, tmp.condition_slice[j].con_H2.attr_value);

            }

            {//for con_V1
                 Setting& frg_conV1 = frgi_cfg[CONFIG_NAME_FRAGMENT_CON_V1];
                 frg_conV1.lookupValue(CONFIG_NAME_FRAGMENT_ISVALID, tmp.condition_slice[j].con_V1.isValid);
                 frg_conV1.lookupValue(CONFIG_NAME_FRAGMENT_V_ATTR_NUM, tmp.condition_slice[j].con_V1.attr_num);
                 frg_conV1.lookupValue(CONFIG_NAME_FRAGMENT_ATTR_PRIKEY, tmp.condition_slice[j].con_V1.attr_prikey);
                 ...
                 {
                     string s = list_cfg[k];
                     tmp.condition_slice[j].con_V1.attr_frag_strlist[k] = s;
                 }
...
void MetadataManager::set_fargment_info(Fragment &frg)
{
    ...
    {
        //if(!tmp.condition_slice[i].isValid )
        {
            //continue;
        }
        ...
        frg_i_cfg.add(CONFIG_NAME_FRAGMENT_ISVALID,Setting::TypeBoolean) = frg.condition_slice[i].isValid;


          //if(frg.condition_slice[i].con_H1.isValid)
          {
          ...
          //for vertical fragment slice metadata store
          {
            ...
            for(int j=0;j<frg.condition_slice[i].con_V1.attr_num;j++)
                list_cfg.add(Setting::TypeString) = frg.condition_slice[i].con_V1.attr_frag_strlist[j];
          }
...
//                  = frg.condition_slice[i].con_V2.isValid;

//          frg_conV2_cfg.add(CONFIG_NAME_FRAGMENT_V_ATTR_NUM,Setting::TypeInt) \
//                  = frg.condition_slice[i].con_V2.attr_num;

//          frg_conV2_cfg.add(CONFIG_NAME_FRAGMENT_ATTR_PRIKEY,Setting::TypeString) \
//                  = frg.condition_slice[i].con_V2.attr_prikey;
//          frg_conV2_cfg.add(CONFIG_NAME_FRAGMENT_ATTR_FRAG_STRLIST,Setting::TypeArray);
//          for(int j=0;j<frg.condition_slice[i].con_V2.attr_num;j++)
//              frg_conV2_cfg.add(Setting::TypeString) = frg.condition_slice[i].con_V2.attr_frag_strlist[j];
//        }
      }
...
void MetadataManager::set_fragment_metadata_toquerytree()
{
    ...
    for(int i=0;i<len;i++)
    {
        ...
        {
            if(tmp.condition_slice[k].isValid) frag_num++;

            if(tmp.condition_slice[k].con_H1.isValid && tmp.condition_slice[k].isValid)
            {
                ...
                sch.table[pos_sch].site[k+1].condition[0] = tmp.condition_slice[k].con_H1.attr_name + \
                        tmp.condition_slice[k].con_H1.attr_condition + tmp.condition_slice[k].con_H1.attr_value;

                sch.table[pos_sch].site[k+1].hcon_list[1].attr = tmp.condition_slice[k].con_H1.attr_name;
                sch.table[pos_sch].site[k+1].hcon_list[1].op = tmp.condition_slice[k].con_H1.attr_condition;
                sch.table[pos_sch].site[k+1].hcon_list[1].section = tmp.condition_slice[k].con_H1.attr_value;



            }
            if(tmp.condition_slice[k].con_H2.isValid && tmp.condition_slice[k].isValid)
            {
                frag_type = frag_type | 0x1;

                sch.table[pos_sch].site[k+1].hcon_list_len =2;

                sch.table[pos_sch].site[k+1].condition[0] += "&" + tmp.condition_slice[k].con_H2.attr_name + \
                        tmp.condition_slice[k].con_H2.attr_condition + tmp.condition_slice[k].con_H2.attr_value;

                sch.table[pos_sch].site[k+1].hcon_list[2].attr = tmp.condition_slice[k].con_H2.attr_name;
                sch.table[pos_sch].site[k+1].hcon_list[2].op = tmp.condition_slice[k].con_H2.attr_condition;
                sch.table[pos_sch].site[k+1].hcon_list[2].section = tmp.condition_slice[k].con_H2.attr_value;
            }

            if(tmp.condition_slice[k].con_V1.isValid && tmp.condition_slice[k].isValid)
            {
                frag_type = frag_type | 0x2;

                int v_num = tmp.condition_slice[k].con_V1.attr_num;
                for(int p=0; p<v_num; p++)
                {
                    sch.table[pos_sch].site[k+1].condition[1] += tmp.condition_slice[k].con_V1.attr_frag_strlist[p];
